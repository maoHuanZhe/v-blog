    ...
    <el-form :model="dataForm"
      ...
      <el-form-item label="菜单级别" prop="rank">
        <el-radio-group v-model="dataForm.rank">
          <el-radio v-for="rank in rankList"
                    :label="rank.parKey"
                    :key="rank.parKey">
            {{ rank.parValue }}
          </el-radio>
...
import {select} from "@/api/operation/category"
export default {
  data () {
    return {
      visible: false,
      ...
      rankList: [
        {parKey:1,parValue:"一级"},
        {parKey:2,parValue:"二级"},
        {parKey:3,parValue:"三级"}
      ],
      typeList: [
        {parKey:0,parValue:"文章"},
        {parKey:1,parValue:"阅读"}
      ],
      categoryList: [],
  ...
  methods: {
    ...
    // 获取目录列表
    getCategorySelect () {
        select(this.dataForm.type).then(rest =>{
            console.log(rest);
        })
      // this.$http({
      //   url: this.$http.adornUrl('/admin/operation/category/select'),
