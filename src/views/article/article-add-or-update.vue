    ...
    <el-form :model="article" label-width="80px" :rules="rules" ref="articleForm">
      ...
      <el-form-item label="展示类型">
        <el-radio-group v-model="article.coverType">
          <el-radio v-for="type in coverTypeList"
                    :key="type.parKey"
                    :label="type.parKey" >
            {{type.parValue}}
          </el-radio>
...
import { treeDataTranslate,getSysParamArr } from '@/utils'
...
export default {
  ...
  data () {
    return {
      ...
      // coverTypeList: getSysParamArr('ARTICLE_COVER_TYPE'),
        coverTypeList: [],
      url: '',
      file: [],
      rules: {
